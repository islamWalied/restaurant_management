...
<div class="flex-col w-full md:flex md:flex-row md:min-h-screen">
    <div @click.away="open = false" class="flex flex-col flex-shrink-0 w-full text-gray-700 bg-white md:w-64 dark:text-gray-200 dark:bg-gray-800" x-data="{ open: false }">
        <div class="flex flex-row items-center justify-between flex-shrink-0 px-8 py-4">
            <a href="{{route("admin.index")}}" class="text-lg font-semibold tracking-widest text-gray-900 capitalize rounded-lg dark:text-white focus:outline-none focus:shadow-outline">Restaurant</a>
    ...
    <!-- Page Content -->
    <main class="m-2 w-full dark:bg-gray-800/50">
        <div class="mx-6 mt-6 mb-6">
            @if (session()->has('danger'))
                <div class="p-4 mb-4 text-sm text-red-600 dark:bg-gray-700 rounded-lg"
                     role="alert">
                    <span class="font-medium">{{ session()->get('danger') }}!</span>
                </div>
            @endif
            @if (session()->has('success'))
                <div class="p-4 mb-4 text-sm text-green-600 dark:bg-gray-700 rounded-lg"
                     role="alert">
                    <span class="font-medium">{{ session()->get('success') }} !</span>
                </div>
            @endif
            @if (session()->has('warning'))
                <div class="p-4 mb-4 text-sm dark:text-indigo-300 dark:bg-gray-700 rounded-lg"
                     role="alert">
